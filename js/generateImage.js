...
(function(){
  ...
  window.generateImage={
    ...
    pixelMeasuringTool:function(width,height,callback){
      ...
      c.cx.fillStyle="orangered";
      c.cx.fillRect(0,0,1,height);
      c.cx.fillRect(0,0,width,1);
      c.cx.fillRect(width-1,0,1,height);
      c.cx.fillRect(0,height-1,width,1);
      this.exportCanvas(c,callback);
    },
    ...
    drawText:function(textString,callback){
      let c=wordsCanvas;
      c.cx=c.getContext("2d");
      c.cx.clearRect(0,0,c.width,c.height);
      
      // console.log(window.devicePixelRatio);
      // let scale=window.devicePixelRatio;
      // c.width=Math.floor(c.width*scale);
      // c.height=Math.floor(c.height*scale);
      // c.cx.scale(scale,scale);

      let w=c.width;
      let h=c.height;
      c.cx.imageSmoothingEnabled=false;
      if(font.wireframe){  // create wireframe background if applicable
        c.cx.fillStyle="CornflowerBlue";
        c.cx.strokeStyle=c.cx.fillStyle;
      ...
      c.cx.font=`${font.style} ${font.variant} ${font.weight} ${parseFloat(font.size)+"px"} ${font.family}`;
      // new - hairline spacing method
      let padding="";
      let hairSpace=String.fromCharCode(8202);
      for(let i=0;i<font.spacing;i++){
        padding=padding+hairSpace;
      }
      let spacedText=textString.split("").join(padding);
      // draw text repeatedly to look pixelated
      for(let i=0;i<font.fix.renderCount;i++){
        c.cx.fillText(spacedText,font.left+font.fix.offset,font.top+font.fix.offset);
      }
      // export
      this.exportCanvas(c,callback);
    },

    drawX:function(width,height,color,callback){
      let c=this.initializeCanvas(width,height);
      let w=c.width;
      let h=c.height;
      c.cx.fillStyle=color;
    ...
    //     if(!textLineWireframesVisible){ wordsBGBox.visible=false; }
